...
- A finding that requires multiple lines of independent evidence to be convincing
- A complete mechanistic story — not just "what happened" but a well-supported explanation of "why it happened"
- Sufficient data and analysis to address the dominant sources of uncertainty
- A contribution where the method, the validation, and the physical interpretation are all part of the story

JGR-Oceans is **not** the right home when:

- The central finding fits within a single sharp message without needing multiple independent evidence lines to be convincing
- The evidence chain is incomplete — a suggestive pattern without supporting diagnostics
...
|---------------|----------------------|
| GRL | Single sharp finding, limited but sufficient evidence, rapid publication |
| JPO | Dynamical equations, scaling analysis, energy budgets as central narrative |
| Nature Communications | Cross-disciplinary significance, narrative-driven writing, broad audience |
| ESSD | Data product description, validation-focused, no scientific hypothesis testing |
| JAOT | Method/algorithm development, technical validation, no scientific finding required |
...
Mechanism language in JGR-Oceans should be matched to the evidence level:

- **Appropriate when supported by process diagnostics:** "Our analysis demonstrates that", "These results establish that", "The mechanism is confirmed by" — justified when the evidence includes direct process-oriented analysis (e.g., energy conversion terms, budget closure, scale analysis).
- **Appropriate when evidence is correlative but suggestive:** "These patterns are consistent with", "The results suggest that", "This indicates" — when the evidence points toward a mechanism but does not directly diagnose it.
- **Inappropriate even in JGR-Oceans:** Claiming causality from spatiotemporal correlation alone without addressing alternative explanations or supporting process diagnostics.
...
- Error estimates are expected for quantitative results.
- Sensitivity tests strengthen claims based on uncertain parameters or thresholds.
- Limitations should be discussed specifically and honestly — which assumptions matter most and what would change if they were violated.
- A dedicated uncertainty or limitations subsection is acceptable and common in JGR-Oceans.
...
JGR-Oceans Introduction is **more developed than in letter-format journals** and follows a structured arc:

- **Paragraph 1:** Establish the broader context — the phenomenon, its importance, and current knowledge (4-6 sentences).
- **Paragraph 2:** Identify the knowledge gap — what is not understood, what evidence conflicts, what methodological limitation has prevented progress.
- **Paragraph 3:** State the objective, approach, and how it addresses the gap.
- **Final paragraph:** Preview key findings and optionally provide a brief roadmap.

A comprehensive but not exhaustive literature review is appropriate, covering the most directly relevant studies.
...
JGR-Oceans Results can be **organized by evidence type or by sub-claim**, not necessarily by figure. Typical approaches:

- **Thematic organization:** One subsection per aspect of the finding (e.g., spatial patterns first, then temporal evolution, then mechanism)
- **Progressive organization:** Descriptive results first (what happens), then diagnostics (why it happens)
- **Combined Results and Discussion:** When used, it should be clearly signaled and each part should still serve its narrative function.

Key principles:
- Lead each paragraph with the finding, then describe the supporting evidence
- Reference figures in the order they build the argument, not necessarily numerical order
- Quantitative results must include units and uncertainty
- Secondary findings can have their own subsections if they contribute to the overall story

### Discussion

JGR-Oceans Discussion plays a **central role in the narrative** and is more comprehensive than in letter-format journals:

1. **Restate main findings** to anchor the discussion
2. **Mechanism interpretation** — what the results mean in physical terms, with process-oriented analysis
3. **Comparison with prior work** — quantitative comparison, explaining reasons for agreement or disagreement
4. **Implications** — what the results mean for the broader field, other regions, or future work
5. **Limitations** — acknowledgment of assumptions, uncertainties, and boundary conditions

The Discussion should not restate Results in detail but should build on them.
...
JGR-Oceans claims are **strong but bounded by the evidence**:

| Evidence level | Language | Example |
|---------------|----------|---------|
| Direct quantitative evidence with uncertainty | "demonstrates", "establishes", "confirms" | "Budget closure demonstrates that lateral advection dominates the heat balance at this location." |
...
In ocean science, the type of process diagnostics determines how strongly a mechanism can be stated:

- **Energy diagnostics** (EKE conversion terms, spectral analysis) support claims about eddy-mean flow interaction and instability processes.
- **Budget closure** (heat, salt, carbon, momentum) supports claims about which processes control variability.
- **Scaling analysis** supports claims about regime transitions and dominant balances.
- **Lagrangian diagnostics** support claims about water mass pathways and transformation.

Without process-oriented diagnostics, mechanism language should be limited to "suggests" and "is consistent with."
...
- The complete evidence chain — each link from data through analysis to interpretation
- Validation results that establish the credibility of the central claim
- The mechanism interpretation that distinguishes this study from a purely descriptive one
- Uncertainty assessment and limitation acknowledgment

### What to cut first if over length

- Extended literature review in Introduction (keep only the most directly relevant references)
- Individual case studies or examples that support but do not advance the central argument
- Secondary validation against datasets that are less relevant to the central claim
- Excessive methodological detail that can move to supplementary material
- Discussion of implications that are speculative without supporting evidence

### Hard constraints to verify

At the final pre-submission stage, check the journal's current author guidelines for:
- Word count limits
- Figure count limits
- Abstract length requirements
- Data availability statement (AGU data policy)
